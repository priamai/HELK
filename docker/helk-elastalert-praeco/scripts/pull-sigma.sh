...
echo -e "${HELK_INFO_TAG} Checking if Elastalert rules folder has SIGMA template rules.."
# There are too many rules therefore best to place it in the sigma home
if ls "${ESALERT_SIGMA_HOME}/rule_templates/" | grep -v "^helk_" >/dev/null 2>&1; then
    echo -e "${HELK_INFO_TAG} [+++++] SIGMA rules available in rules folder.."
    SIGMA_RULES_AVAILABLE=YES
else
    echo -e "${HELK_INFO_TAG} [+++++] SIGMA rules not available in template rules folder.."
fi
...
function getUpdates() {
    ...
    if [[ ${LOCAL} = ${REMOTE} ]]; then
        ...
        if [[ ${SIGMA_RULES_AVAILABLE} == "YES" ]]; then
            ...
            find "${ESALERT_SIGMA_HOME}/rule_templates/" -type f -not -name "helk_*" -delete
            find "${ESALERT_SIGMA_HOME}/rule_templates/" -type f -not -name "custom_*" -delete
        fi
    ...
    if [[ "$rule_category" == "rules/windows/process_creation" ]]; then
        for rule in "${rule_category}"/* ; do
            if [[ ${rule} != "rules/windows/process_creation/win_mal_adwind.yml" ]]; then
                if SIGMAremoveNearRules "$rule"; then
                    ...
                    tools/sigmac -t elastalert -c tools/config/generic/sysmon.yml -c "${helk_sigmac}" --backend-option timestamp_field=etl_processed_time -o "${ESALERT_SIGMA_HOME}"/rule_templates/sigma_sysmon_"$(basename "${rule}")" "$rule"
                    # Give unique rule name for sysmon
                    sed -i 's/^name: /name: Sysmon_/' "${ESALERT_SIGMA_HOME}"/rules/sigma_sysmon_"$(basename "${rule}")"
                    tools/sigmac -t elastalert -c tools/config/generic/windows-audit.yml -c "${helk_sigmac}" --backend-option timestamp_field=etl_processed_time -o "${ESALERT_SIGMA_HOME}"/rule_templates/sigma_"$(basename "${rule}")" "$rule"
        ...
        for rule in "${rule_category}"/* ; do
            if SIGMAremoveNearRules "$rule"; then
                ...
                tools/sigmac -t elastalert -c "${helk_sigmac}" --backend-option timestamp_field=etl_processed_time -o "${ESALERT_SIGMA_HOME}"/rule_templates/sigma_"$(basename "${rule}")" "$rule"
                rule_counter=$[$rule_counter +1]
            fi
        done
    fi
...
for rule in rules/apt/* ; do
    ...
    if SIGMAremoveNearRules "$rule"; then
        ...
        tools/sigmac -t elastalert -c tools/config/generic/sysmon.yml -c "${helk_sigmac}" --backend-option timestamp_field=etl_processed_time -o "${ESALERT_SIGMA_HOME}"/rule_templates/sigma_sysmon_apt_"$(basename "${rule}")" "$rule"
        # Give unique rule name for sysmon
        sed -i 's/^name: /name: Sysmon_/' "${ESALERT_SIGMA_HOME}"/rules/sigma_sysmon_apt_"$(basename "${rule}")"
        tools/sigmac -t elastalert -c tools/config/generic/windows-audit.yml -c "${helk_sigmac}" --backend-option timestamp_field=etl_processed_time -o "${ESALERT_SIGMA_HOME}"/rule_templates/sigma_apt_"$(basename "${rule}")" "$rule"
        rule_counter=$[$rule_counter +1]
    fi
done
echo "-------------------------------------------------------"
...
rule_counter=$(find ${ESALERT_SIGMA_HOME}/rule_templates/ -type f | wc -l)
echo "-------------------------------------------------------"
echo -e "${HELK_INFO_TAG} [+++] Finished loading $rule_counter SIGMA rules"
echo "-------------------------------------------------------"
echo " "
fi
...
echo "-------------------------"
rule_counter=0
for ef in ${ESALERT_SIGMA_HOME}/rule_templates/* ; do
    if [[ -s ${ef} ]]; then
        continue
    else
...
echo "------------------------------------------------------------------"
for er in ${ESALERT_SIGMA_HOME}/rule_templates/*; do
    ...
    if [[ "$counter" == "2" ]] ; then
        ...
        awk -v RS= -v filename="$name" '{print > ("/etc/elastalert/rule_templates/"filename NR ".yml")}' ${er}
